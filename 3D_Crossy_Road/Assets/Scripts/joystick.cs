...
public class joystick : MonoBehaviour, IPointerDownHandler, IPointerUpHandler, IDragHandler
{
    ...
    void Start()
    {
        Mypos = go_Player.transform.position;
    ...
    void FixedUpdate()
    {
        if (isTouch) //터치 중일 때만
        {
            ...
            go_Player.GetComponent<Rigidbody>().AddForce(value*moveSpeed);
        }
    }

    private void LateUpdate()
    {
        CamMoveValue = go_Player.transform.position - Mypos;
        print(CamMoveValue);
        Camera.transform.position=Vector3.Lerp(Camera.transform.position,new Vector3(CamMoveValue.x,Camera.transform.position.y,CamMoveValue.z-8.5f),Time.deltaTime*Camspeed);
        Light.transform.position=new Vector3(Light.transform.position.x,Light.transform.position.y,CamMoveValue.z+40);
    }
    ...
    public void OnDrag(PointerEventData eventData) //드래그할 때 조이스틱이 움직이도록
    {
        ...
        value = Vector3.ClampMagnitude(value, radius); //value를 radius안에 가둠
