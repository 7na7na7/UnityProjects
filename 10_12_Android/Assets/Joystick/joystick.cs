...
public class joystick : MonoBehaviour, IPointerDownHandler, IPointerUpHandler, IDragHandler
{
    ...
    private Vector2 movePosition; //움직일 좌표 저장
    private Vector2 value;
    
    private Player[] go_Player; //움직일 플레이어
    private Player pl;
    ...
    void Start()
    {
        ...
        go_Player = FindObjectsOfType<Player>();
        for (int i = 0; i < go_Player.Length; i++)
        {
            if (go_Player[i].pv.IsMine) //플레이어의 ismine이 true라면
            {
                pl = go_Player[i];
    ...
    void Update()
    {
        if (isTouch) //터치 중일 때만
        {
            pl.transform.position += (Vector3)movePosition; //이동
            if (movePosition.x < 0)
            {
                pl.pv.RPC("FlipXRPC",RpcTarget.AllBuffered); //재접속시 flix를 동기화해주기 위해 AllBuffered
               
            }
            else
            {
                pl.pv.RPC("FlipXRPC2",RpcTarget.AllBuffered); //재접속시 flix를 동기화해주기 위해 AllBuffered
            }
            pl.ismove = true;
        }
        else
