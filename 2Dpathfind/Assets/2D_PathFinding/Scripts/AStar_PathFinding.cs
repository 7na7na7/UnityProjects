...
public class AStar_PathFinding : MonoBehaviour
{
    ...
    IEnumerator Move(int count)
    {
        ...
        {
            Vector2 v2 = new Vector2(FinalNodeList[i].x, FinalNodeList[i].y) - (Vector2) Charactor.transform.position;
            dir = (Vector3) v2;
            v2.Normalize();
            yield return new WaitUntil(() => Vector3.Distance(new Vector3(FinalNodeList[i].x, FinalNodeList[i].y,0),Charactor.transform.position)<0.1f);
        }
        //Charactor.transform.position=new Vector3(Mathf.RoundToInt(Charactor.transform.position.x),Mathf.RoundToInt(Charactor.transform.position.y),0);
        dir = Vector3.zero;
    }
