...
//Mathf.Clamp(-10,0,100)일 경우 값은 0이다.
    ...
    {
        while (theCamera.orthographicSize<7.5f)
        {
    ...
    {
        while (theCamera.orthographicSize>5.5f)
        {
            theCamera.orthographicSize -= 0.1f;
            yield return new WaitForSeconds(delay);
        }
    ...
    IEnumerator closeUpCor()
    {
        ...
        {
            while (theCamera.orthographicSize>6.8f)
            {
                theCamera.orthographicSize -= 0.1f;
                yield return new WaitForSeconds(delay*0.1f);
            }
        ...
        {
            while (theCamera.orthographicSize>4.8f)
            {
                theCamera.orthographicSize -= 0.1f;
                yield return new WaitForSeconds(delay*0.1f);
            }
...
//Mathf.Clamp(-10,0,100)일 경우 값은 0이다.
    ...
    public IEnumerator targetChange(GameObject g)
    {
        ...
        theCamera.orthographicSize = 5.5f;
        target = posGO;
        Time.timeScale = 0.1f;
        while (theCamera.orthographicSize > 4f)
        {
