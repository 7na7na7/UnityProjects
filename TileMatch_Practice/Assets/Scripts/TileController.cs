...
public class TileController : MonoBehaviour
{
    ...
    // 타일 찾기 부분(astar 알고리즘)
    public static Vector2Int bottomLeft, topRight, startPos, targetPos; //왼쪽아래, 오른쪽끝으로 그리드 크기 지정
    public static List<Node> FinalNodeList; //최종 길찾기 리스트

    static int sizeX, sizeY; //그리드 사이즈
    static Node[,] NodeArray; //2차원 노드 배열
    static Node StartNode, TargetNode, CurNode;
    static List<Node> OpenList, ClosedList; //열린리스트, 닫힌리스트
